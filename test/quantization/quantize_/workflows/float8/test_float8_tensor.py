...
class TestFloat8Tensor(TestCase):
    ...
    def test_slice(self):
        ...
        self.assertEqual(
            weight1.original_weight_tensor._data,
            dummy.weight.original_weight_tensor._data.narrow(0, 0, 64),
        )
        self.assertEqual(
            weight1.original_weight_tensor.scale,
            dummy.weight.original_weight_tensor.scale.narrow(0, 0, 64),
        )
        self.assertEqual(
            weight2.original_weight_tensor._data,
            dummy.weight.original_weight_tensor._data.narrow(1, 0, 128),
        )
        self.assertEqual(
            weight2.original_weight_tensor.scale,
            dummy.weight.original_weight_tensor.scale,
        )
    ...
    def test_slice_and_copy_(self):
        ...
        assert (
            param.data.original_weight_tensor._data.data_ptr()
            == param_data.original_weight_tensor._data.data_ptr()
        )
        assert (
            param.data.original_weight_tensor.scale.data_ptr()
            == param_data.original_weight_tensor.scale.data_ptr()
        )
        orig_value = param.data.original_weight_tensor._data[0][0].item()
        ...
        assert param.data.original_weight_tensor._data[0][0] != orig_value
    ...
    def test_cat(self):
        ...
        self.assertEqual(
            dummy1.weight.original_weight_tensor._data,
            cat_qweight1.original_weight_tensor._data,
        )
        self.assertEqual(
            dummy1.weight.original_weight_tensor.scale,
            cat_qweight1.original_weight_tensor.scale,
        )
        ...
        ref_data = torch.cat(
            [
                linear1.weight.original_weight_tensor._data,
                linear2.weight.original_weight_tensor._data,
            ],
            dim=1,
        )
        ref_scale = linear1.weight.original_weight_tensor.scale
        self.assertEqual(cat_qweight2.original_weight_tensor._data, ref_data)
        self.assertEqual(cat_qweight2.original_weight_tensor.scale, ref_scale)
